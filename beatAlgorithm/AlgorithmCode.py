...
# !!! ADJUST THESE FOR VIABLE BPM !!!
maxBPM = 160 # ! IF 160 BPM DOES NOT WORK YOU CAN CHANGE IT HERE !
BPMbounds = [maxBPM/2, maxBPM] # Right now this is 80 BPM minimum and 160 maximum
...
pathToImport = "test.mp3" # Import as .wav or .mp3; IF .WAV MAKE SURE ITS 16 BIT
...
signal = np.frombuffer(signal, dtype ="int16") # total time should be correct now
...
goodBPM = False
while(goodBPM == False):
  if (BPM >= BPMbounds[0]) and (BPM <= BPMbounds[1]):
    goodBPM = True
  if BPM >= BPMbounds[1]:
    BPM = BPM / 2
  if BPM < BPMbounds[0]:
